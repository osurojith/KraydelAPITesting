        ...
        if (status_code.equals("20000")) {
            int count=0;
                ...
                for (int i = 1; i <= jsonPath.getList("content.base-stations").size(); i++) {
                    ...
                    String status = jsonPath.getString("content.base-stations[" + val + "].status");
                    get_db_data(besestationid);
                    while (results.next()) {
                        count++;
                    ...
                    if (jsonPath.getString("content.base-stations[" + val + "].status").equalsIgnoreCase("ASSIGNED")) {
                        ...
                        for (int x = 1; x <= jsonPath.getList("content.base-stations[" + val + "].grampas").size(); x++) {
                            ...
                            Assert.assertEquals("Validate base_station.id", results.getString("grampaid"), EncryptionServiceImpl.decryptToLong(elderid).toString());
                            Assert.assertEquals("Validate base_station.devicekey", results.getString("fname"), fname);
                            Assert.assertEquals("Validate base_station.status", results.getString("lname"), lname);
                            Assert.assertEquals("Validate base_station.status", results.getString("locationid"), EncryptionServiceImpl.decryptToLong(locationid).toString());
                        }
